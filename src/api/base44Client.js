...
function entityClient(entityName) {
  return {
    list: (sort = '-created_date', limit = 500) => request(`/api/entities/${entityName}/list`, {
      method: 'POST',
      body: JSON.stringify({ sort, limit }),
    }),
    filter: (query = {}, sort = '-created_date', limit = 500) => request(`/api/entities/${entityName}/filter`, {
      method: 'POST',
      body: JSON.stringify({ query, sort, limit }),
    }),
    get: (id) => request(`/api/entities/${entityName}/get/${id}`),
    create: (data) => request(`/api/entities/${entityName}/create`, {
      method: 'POST',
      body: JSON.stringify(data),
    }),
    bulkCreate: (items) => request(`/api/entities/${entityName}/bulk-create`, {
      method: 'POST',
      body: JSON.stringify({ items }),
    }),
    update: (id, data) => request(`/api/entities/${entityName}/update/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
    bulkUpdate: (items) => request(`/api/entities/${entityName}/bulk-update`, {
      method: 'PATCH',
      body: JSON.stringify({ items }),
    }),
    updateMany: (query, update) => request(`/api/entities/${entityName}/update-many`, {
      method: 'PATCH',
      body: JSON.stringify({ query, update }),
    }),
    delete: (id) => request(`/api/entities/${entityName}/delete/${id}`, { method: 'DELETE' }),
    deleteMany: (query) => request(`/api/entities/${entityName}/delete-many`, {
      method: 'POST',
