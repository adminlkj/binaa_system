...
export default function ClientPayments() {
  ...
  const reverse = async (item) => {
    ...
    try {
      ...
      const jes = allJE.filter(je => je.sourceType === 'ClientPayment' && (je.entryNo || '').includes(item.paymentNo));
    ...
    >
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder={t('بحث بالعميل أو رقم السند...', 'Search client or receipt no...', lang)} className="ps-9" />
        </div>
