...
export default function CostCenters() {
  const { lang } = useStore();
  const [data, setData] = useState({ projects: [], expenses: [], subInvoices: [], salesInvoices: [] });
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [projects, expenses, subInvoices, salesInvoices] = await Promise.all([
  ...
  const exportColumns = [
    { header: { ar: 'مركز التكلفة', en: 'Cost Center' }, value: (r) => r.name || (lang === 'ar' ? 'غير مخصّص' : 'Unassigned') },
    ...
    { header: { ar: 'مقاولو الباطن', en: 'Subcontractors' }, value: (r) => r.subCost },
  ...
  return (
    <ModuleLayout
      title={t('تحليل مراكز التكلفة', 'Cost Center Analysis', lang)}
      subtitle={t('الإيراد والتكلفة والهامش لكل مركز تكلفة (مشروع)', 'Revenue, cost and margin per cost center', lang)}
      actions={
        <div className="flex items-center gap-2">
          <TableToolbar
            columns={exportColumns}
            rows={analysis.rows}
            title={{ ar: 'تحليل مراكز التكلفة', en: 'Cost Center Analysis' }}
      ...
      <Card>
        <div className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>{t('مركز التكلفة', 'Cost Center', lang)}</TableHead>
            ...
            <TableBody>
              {loading ? Array.from({ length: 5 }).map((_, i) => <TableRow key={i}>{Array.from({ length: 7 }).map((_, j) => <TableCell key={j}><div className="h-4 bg-muted animate-pulse rounded" /></TableCell>)}</TableRow>)
                : analysis.rows.length === 0 ? <TableRow><TableCell colSpan={7} className="text-center py-10 text-muted-foreground flex-col"><PieChart className="size-7 mx-auto mb-2 text-indigo-400/50" />{t('لا توجد بيانات مراكز تكلفة', 'No cost center data', lang)}</TableCell></TableRow>
