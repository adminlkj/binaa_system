...
import { Plus, Search, Pencil, Trash2, RefreshCw, Truck } from 'lucide-react';
...
export default function Equipment() {
  ...
  const load = async () => { setLoading(true); setItems(await base44.entities.Equipment.list('-created_date')); setLoading(false); };
  useEffect(() => { load(); }, []);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {Object.entries(EQUIPMENT_STATUS).map(([status, cfg]) => (
          <button key={status} onClick={() => setFilterStatus(filterStatus === status ? 'ALL' : status)}
            className={`p-3 rounded-xl border text-center transition-all ${filterStatus === status ? 'ring-2 ring-cyan-500' : ''} ${cfg.color.replace('border border-', 'border ')}`}>
            <p className="text-xl font-bold">{statusCounts[status] || 0}</p>
            <p className="text-xs mt-0.5">{lang === 'ar' ? cfg.ar : cfg.en}</p>
          </button>
        ))}
      </div>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={8} className="text-center py-10 text-muted-foreground">{t('لا توجد معدات', 'No equipment found', lang)}</TableCell></TableRow>
              ) : filtered.map(item => {
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4 py-2">
            ...
            <div className="space-y-1.5">
              <Label>{t('تاريخ الشراء', 'Purchase Date', lang)}</Label>
