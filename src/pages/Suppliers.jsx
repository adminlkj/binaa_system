...
export default function Suppliers() {
  ...
  const load = async () => { setLoading(true); setItems(await base44.entities.Supplier.list('-created_date')); setLoading(false); };
  useEffect(() => { load(); }, []);
  ...
  const fields = [
    ['code', t('الكود', 'Code', lang)], ['name', t('الاسم', 'Name', lang)],
  ...
  return (
    <ModuleLayout
      ...
      actions={<Button onClick={openNew} className="gap-2"><Plus className="size-4" />{t('مورد جديد', 'New Supplier', lang)}</Button>}
    >
          ...
          <Table>
            ...
            <TableBody>
              {loading ? Array.from({ length: 3 }).map((_, i) => <TableRow key={i}>{Array.from({ length: 7 }).map((_, j) => <TableCell key={j}><div className="h-4 bg-muted animate-pulse rounded" /></TableCell>)}</TableRow>)
                : filtered.length === 0 ? <TableRow><TableCell colSpan={7} className="text-center py-10 text-muted-foreground">{t('لا يوجد موردون', 'No suppliers', lang)}</TableCell></TableRow>
                : filtered.map(item => (
